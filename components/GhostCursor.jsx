...
const GhostCursor = ({
  ...
  fadeDelayMs = 200,
  fadeDurationMs = 1000,
  zIndex = -1 // Negative z-index to stay behind content
}) => {
  ...
  const effectiveBrightness = isTouch ? brightness * 1.5 : brightness;
  const effectiveBloomStrength = isTouch ? bloomStrength * 1.5 : bloomStrength;
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      vec2 mouse = (iMouse * 2.0 - 1.0) * iResolution.xy / min(iResolution.x, iResolution.y);
      ...
      for (int i = 0; i < MAX_TRAIL_LENGTH; i++) {
        vec2 pm = (iPrevMouse[i] * 2.0 - 1.0) * iResolution.xy / min(iResolution.x, iResolution.y);
        float t = 1.0 - float(i) / float(MAX_TRAIL_LENGTH);
        t = pow(t, 2.0);
        
        if (t > 0.01) {
          vec4 bt = blob(uv, pm, t * 0.8, iOpacity);
          // Use volumetric alpha blending to prevent blown-out circles when points stack
          colorAcc = colorAcc + bt.rgb * max(0.0, 1.0 - alphaAcc);
          alphaAcc = alphaAcc + bt.a * max(0.0, 1.0 - alphaAcc);
        }
      }

      colorAcc *= iBrightness;
      
      // Prevent massive bloom blow-out when trail points stack up (e.g. at screen edges)
      // We scale down the color preserving its hue if it exceeds the bloom threshold heavily.
      float maxCol = max(colorAcc.r, max(colorAcc.g, colorAcc.b));
      if (maxCol > 1.2) {
          colorAcc *= (1.2 / maxCol);
      }
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        iBrightness: { value: effectiveBrightness },
    ...
    const bloomPass = new UnrealBloomPass(new THREE.Vector2(1, 1), effectiveBloomStrength, bloomRadius, bloomThreshold);
    ...
    const animate = () => {
      ...
      if (pointerActiveRef.current) {
        ...
        if (isTouch) {
          // On mobile, gently drift to center when finger is lifted
          mat.uniforms.iMouse.value.lerp(new THREE.Vector2(0.5, 0.5), 0.05);
          // Keep it highly visible
          fadeOpacityRef.current = Math.max(0.85, fadeOpacityRef.current - 0.01);
        } else {
  ...
  useEffect(() => {
    if (materialRef.current) {
      materialRef.current.uniforms.iBrightness.value = effectiveBrightness;
    }
  }, [effectiveBrightness]);
